...
//  Make many (let’s say 3-5) Circles and expand them slowly like you are filling air in the
// balloon, and then shrinking them together.
...
void expand(char c, int radius){
    ...
    for(int i=1; i<=radius-3; i++){
        circle(c, i, space);
        circle(c, i+1, space-1);
        circle(c, i+2, space-2);
        sleep(1000);
        circle(' ', i, space);
        circle(' ', i+1, space-1);
        circle(' ', i+2, space-2);
